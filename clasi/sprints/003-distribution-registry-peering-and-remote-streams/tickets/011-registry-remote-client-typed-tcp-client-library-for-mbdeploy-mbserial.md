...
- [ ] New module `mbtools/registry/remote_client.py`,
...
- [ ] Methods: `find(uid)`, `lock(uid, kind)`, `unlock(uid)`, `flash(uid,
...
- [ ] Same exception hierarchy as `registry.client`
...
- [ ] If `--auth-token` is in play (Decision 6), the client sends it as
      the connection's first message, matching ticket 006's server-side
      expectation.
- [ ] `mtools.registry.render` (ticket 010) is not a dependency of this
