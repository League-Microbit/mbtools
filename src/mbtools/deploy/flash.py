"""deploy.flash — pyOCD flash with transient retry and mass-erase recovery.

Ported near-verbatim from today's ``mbdeploy``'s own
``src/mbdeploy/flash.py`` (see ticket 005 in this sprint): the one place
pyOCD's failure wording is matched against known signatures, unchanged
in shape from the existing, hardware-proven implementation.
``docs/acceptance/001-hardware.md``'s hodr/braeburn transient-probe-
timeout retries and braeburn's ``0x67`` mass-erase-recovery finding are
exactly what this module handles.

This module takes no lock itself and knows nothing about the registry
-- it flashes whatever UID it's given, streaming log lines to its
caller. Ticket 007 is the caller that takes the ``flash``-kind lock via
``registry.client`` before invoking this module, and calls
``mark_flashed`` (ticket 003) after.
