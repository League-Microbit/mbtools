...
its name then no longer says where it is. ``registry.store``'s name-registry
methods (sprint 004, ticket 001) are what record the exceptions, and this
module is what they call to compute the default in the first place. Nothing
here knows about overrides -- keeping the mapping pure is what lets its
digest stay a cross-repo contract.

Ported near-verbatim (sprint 004, ticket 001) from
``microbit-radio-relay/server/src/mbrelay/naming.py`` -- pure functions, no
behaviour change, checked against that repo's published sha256
canonical-form digest (see ``tests/relay/test_naming.py``).

Normative spec: ``docs/design/radio-addressing.md`` in radio-robot-lib (its
wiki's "Radio addressing" page). ``tests/relay/radio-address-vectors.json``
