"""name -> (channel, group) is a contract between the relay firmware,
``mbrelay`` and the robot. The normative spec is
``docs/design/radio-addressing.md`` in radio-robot-lib;
``radio-address-vectors.json`` transcribes its digests and vectors, and these
tests assert against that file -- the whole 3125-name space via its digest --
never against prose.

Ported near-verbatim (sprint 004, ticket 001) from
``microbit-radio-relay/server/tests/test_naming.py``, against the same
``radio-address-vectors.json`` fixture, copied alongside this file -- the
acceptance criterion is that ``mbtools.relay.naming`` passes the original
repo's canonical-form sha256 test vector unchanged.
"""
...
from mbtools.relay.naming import (
    address,
    decode,
    encode,
    name_to_radio,
